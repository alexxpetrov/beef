...
type ServiceChatCache interface {
	GetLastMessagesFromRoom(ctx context.Context, roomID gocql.UUID) ([]domain.Message, error)
...
func (h *Handler) JoinRoom(w http.ResponseWriter, r *http.Request) {
	...
	roomID, err := gocql.ParseUUID(paramRoomId)

	if err != nil {
		common.ProcessError(w, "failed to parse userID using gocql.ParseUUID(paramUserID)", http.StatusBadRequest)
		return
	}
	...
	if paramUserID == "" {
		paramUserID = r.URL.Query().Get("user_id")
	}

	userID, err := gocql.ParseUUID(paramUserID)
	...
	fmt.Println("h.chatCache.GetLastMessagesFromRoom", messages)
...
func (h *Handler) GetClients(w http.ResponseWriter, r *http.Request) {
	...
	users, err := h.chatCache.GetRoomClients(r.Context(), roomID.String())
	if err != nil {
