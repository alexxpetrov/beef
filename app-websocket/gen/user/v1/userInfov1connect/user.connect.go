...
var (
	userInfoServiceServiceDescriptor           = v1.File_user_v1_user_proto.Services().ByName("UserInfoService")
...
func NewUserInfoServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) UserInfoServiceClient {
	baseURL = strings.TrimRight(baseURL, "/")
	return &userInfoServiceClient{
		getUserInfo: connect.NewClient[v1.GetInfoRequest, v1.GetInfoResponse](
			httpClient,
			baseURL+UserInfoServiceGetUserInfoProcedure,
...
// userInfoServiceClient implements UserInfoServiceClient.
...
// NewUserInfoServiceHandler builds an HTTP handler from the service implementation. It returns the
// path on which to mount the handler and the handler itself.
//
// By default, handlers support the Connect, gRPC, and gRPC-Web protocols with the binary Protobuf
// and JSON codecs. They also support gzip compression.
func NewUserInfoServiceHandler(svc UserInfoServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	userInfoServiceGetUserInfoHandler := connect.NewUnaryHandler(
		UserInfoServiceGetUserInfoProcedure,
		svc.GetUserInfo,
		connect.WithSchema(userInfoServiceGetUserInfoMethodDescriptor),
...
// UnimplementedUserInfoServiceHandler returns CodeUnimplemented from all methods.
